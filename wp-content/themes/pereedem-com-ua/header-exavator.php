...
<body>
<?php ?>
<section id="header" class="exavator-page" style="background-image: url('<?php the_post_thumbnail_url(); ?>');">
...
//                        'category__in' => 3, //из какой категории вывести (удалите эту строку, если хотите, чтобы показовало последние материалы из всех рубрик сразу)
        ...
        if ($my_query->have_posts()) {
            while ($my_query->have_posts()) {
                ...
                $alt = $image['alt'] ?>
                <a href="<?php the_permalink(); ?>" class="exavators-list__item">
							<span class="exavators-list__image">
							    <img src="<?php the_field('kartinka_dlya_glavnoj');?>" alt="<?php the_field('opisanie_ekskavatora') ?>">
                            </span>
                    <h3><?php the_title(); ?></h3>
                    <div class="exavators-list__mobile">
                        <div class="exavators-list__mobile-price">
                            <?php the_field('stoimost_chasa') ?> <span>грн/час</span>
                        </div>
                        <div class="exavators-list__mobile-price">
                            <?php the_field('stoimost_smeny') ?> <span>грн/смена</span>
                        </div>
                    </div>
                    <table>
                        <tr>
                            <td>Стоимость часа</td><td><?php the_field('stoimost_chasa') ?></td>
                        </tr>
                        <tr>
                            <td>Стоимость смены</td><td><?php the_field('stoimost_smeny') ?></td>
                        </tr>
                    </table>
                </a>
            <?php }
